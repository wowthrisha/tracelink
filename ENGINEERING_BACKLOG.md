...
**Current status (2026-08-09, V24.0 — the authoritative, up-to-date figure; see "Reconciled totals" below for the full item-by-item recount)**: 48 items total — **31 closed**, 7 deferred (reasoned), 3 reviewed-not-implemented, 1 justified-not-changed, **6 open** (ENG-033, ENG-034, ENG-038, ENG-044 each blocked on a named external decision; ENG-046 partially fixed with a real, quantified, low-risk-to-runtime remainder; **ENG-048 is a genuine, High-severity, confirmed-real defect in the Reading Intelligence timer's pause/resume behavior, root cause not yet pinned — the top-priority item for the next engineering session**). Zero Critical defects; **one unresolved High defect (ENG-048)**, found and documented, not fixed, this sprint. Zero contradictory statuses as of this reconciliation pass.
...
- **Not fixed this pass — explicitly deferred, not silently dropped**: `backend/tests/` still has **206 real violations** under the pinned ruleset (mostly F401 unused imports and F841 unused variables accumulated across many test files over many sprints, plus 2 non-functional `# noqa: <free text>` comments in `test_worker_tasks.py` that were never valid ruff syntax — `# noqa` requires exact rule codes, not prose — so they never actually suppressed anything). This means **CI's exact `ruff check backend/app backend/tests` command will still fail** until that debt is addressed. Per this project's own established discipline against blind `--fix` across many files without individual review (ENG-013's explicit precedent), and given the size of the remaining blast radius (many files, zero runtime impact — these are all lint-only, not logic bugs), fixing all 206 in this pass was judged out of proportion for a single sprint continuation; the config pin at least stops the count from silently growing with future ruff releases.
...
- **Affected files**: `backend/ruff.toml` (new), `backend/app/routers/documents.py`, `backend/app/routers/viewer.py`, `backend/app/routers/links.py`, `backend/app/services/reading_analytics_service.py`, `backend/app/models/{document,event,group,link}.py`
- **Regression risk**: Low — verified via isolated review of every change (no blind auto-fix), full backend suite re-run after (1751 passed/1 skipped/0 failed, unchanged), `backend/app` ruff-clean confirmed post-fix.
- **Priority**: 46
- **Status**: **Partially fixed (V24.0)** — `backend/app` fully clean and now protected by a pinned config; `backend/tests`' 206 remaining violations tracked here as real, quantified, low-risk-to-runtime debt, not fixed this pass
- **Owner**: Engineering (V24.0) — `backend/app` portion closed; `backend/tests` cleanup remains open, low priority (no runtime impact, needs the same per-file review discipline at a larger scale)
- **Verification method**: Tool-execution Verified (`ruff check --show-settings`, before/after counts) + Source Verified (each of the 23 fixes individually read and justified, not mechanically applied) + Regression Verified (backend suite 1751 passed/1 skipped/0 failed unchanged)
...
- **Root cause — not fully pinned down, documented honestly rather than guessed**: read `useReadingAnalytics.js` in full (511 lines). `_pause()`/`_accumulate()`/the display `setInterval` all read correctly on paper — `_pause()` calls `_accumulate()` (which does `s.totalActiveMs += delta`, never a reset) before setting `isActive = false`, and the display interval computes `liveActive = s.totalActiveMs` (no addition) while paused, which should show the frozen value, not zero. No line anywhere in the file sets `totalActiveMs` back to `0` after initialization. Attempted to intercept `window.SecureDocAPI.batchReadingEvents` (called by `_flush()`, which reads `s.totalActiveMs` directly) to observe the ref value independent of the display layer, but no flush fired within the observation window, leaving this specific angle inconclusive. Given the mandate's explicit instruction not to fix subtle timing/state bugs based on intuition, and that a live DevTools breakpoint session (not available via browser automation alone) is the appropriate next step to pin the exact line, **no fix was attempted this pass** — the behavioral defect is confirmed with high confidence; the exact faulty statement is not.
- **Impact**: every reader who alt-tabs or switches windows during a reading session sees their visible reading-time counter incorrectly drop to zero and restart — directly undermining trust in a feature whose entire value proposition is accurate reading analytics. Whether the *submitted* (backend-persisted) `total_active_ms` is also undercounted, or whether this is display-only, was not conclusively determined this pass (see above) — but even if display-only, this is a real, user-visible correctness defect in the app's flagship feature.
- **Severity**: **High** — not a security issue, but a core-functionality correctness defect in the product's most-emphasized feature, with a real (if not fully quantified) risk of also corrupting persisted analytics data, which downstream uploader-facing aggregates depend on.
- **Affected files**: `frontend/src/hooks/useReadingAnalytics.js` (suspected), `frontend/src/components/ReadingStatusBar.jsx` (where the symptom renders)
- **Regression risk**: N/A — no fix applied this pass.
- **Priority**: 48 (High severity — should be the next item picked up, ahead of the Low/Medium decision-blocked items)
- **Dependencies**: needs a live browser DevTools debugging session (breakpoints in `_pause`/`_accumulate`/the display interval) to pin the exact faulty line before a confident fix can be written — browser automation alone (Playwright) cannot set breakpoints or inspect React ref internals directly.
- **Status**: **Open — confirmed real defect, root cause not yet pinned, no fix attempted** (V24.0, 2026-08-09)
- **Owner**: Engineering — needs a live debugging session as the next step, not more source-reading (511 lines already read in full without finding the exact line)
- **Verification method**: Browser Verified (5+ independent reproductions via Playwright against the live Railway app, each ruling out a different alternative explanation — remount, network, timing coincidence, interaction-event noise) + Source Verified (full read of `useReadingAnalytics.js`, no obvious fault line found, honestly reported as such rather than guessed at)
...
| ENG-046 | CI's `ruff check` unconfigured; fails under current ruff (`backend/app` fixed, `backend/tests` debt remains) | Low-Medium | 46 | **Partially fixed (V24.0)** — counted as Open below |
| ENG-047 | `CHANGELOG.md` stale since V10.0 (14 sprints undocumented there) | Low | 47 | **Closed — pointer added (V24.0)** |
| ENG-048 | Reading Intelligence active-time counter resets (not pauses) on window blur | High | 48 | Open — confirmed, root cause not pinned, needs live debugging |

**Reconciled totals (2026-08-09, V24.0 — recounted directly from the table above, one item at a time, superseding all prior totals paragraphs below).** V24.0's Step 1 reconciliation found and fixed a real pre-existing bug in this section: **ENG-037's own detail entry has read "Closed" since V22.0 (2026-08-05), but this summary table's row and every totals paragraph below it kept saying "Open"** — carried forward unverified from V22.0 through V23.0. Corrected the table row above. V24.0's Step 4 repository re-sweep then added ENG-046 (partially fixed — counted as Open, real remaining `backend/tests` debt) and ENG-047 (closed). V24.0's Step 5/6/7 Reading Intelligence re-certification then found ENG-048 (High severity, confirmed real, not yet fixed — see above). The counts below reflect all three additions.

- **Total: 48 items** (ENG-001 through ENG-048; ENG-040 is a verification-only audit deliverable with no severity rating, counted once)
- **Closed: 31** — ENG-001,002,003,004,006,007,008,009,010,013,014,017,018,019,020,021,024,029,030,031,032,035,036,037,039,040,041,042,043,045,047
...
- **Open (needs product/ops/design decision, a still-unresolved external blocker, remaining low-risk cleanup work, or a confirmed defect needing further live debugging before a fix): 6** — ENG-033, ENG-034, ENG-038, ENG-044, ENG-046, **ENG-048**

`31 + 7 + 3 + 1 + 6 = 48`. Zero unexplained entries: every item above is FIXED (closed), PROVEN FALSE (closed — no defect found / no longer reproducible), VERIFIED AND DEFERRED WITH A SPECIFIC REASON (deferred / reviewed-not-implemented / justified-not-changed), CLASSIFIED AS A PRODUCT/INFRASTRUCTURE DECISION (open, with a decision record for ENG-033/034 in `docs/governance/`), PARTIALLY FIXED WITH THE REMAINDER EXPLICITLY QUANTIFIED (ENG-046), or CONFIRMED REAL BUT NOT YET ROOT-CAUSED (ENG-048 — the one item this reconciliation cannot mark closed or explain away; flagged as the top-priority next action). No issue is untracked; no duplicate IDs exist; no closed issue appears open and no open issue appears closed anywhere else in this file as of this reconciliation.
