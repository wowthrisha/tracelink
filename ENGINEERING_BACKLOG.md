...
- **Fix (2026-08-13, autonomous residual-issue pass)**: (1) `test_validate_has_rate_limit_decorator` — added `assert has_429 or all(s == 404 for s in responses)`, matching the comment's originally-stated intent exactly. (2) `test_conversion_failure_document_marked_error_by_worker` — **not** the originally-recommended `mock_mark_error.assert_called_once()**`. Traced the actual call chain first: `_mark_document_error` is called by `_process_document_async` (`tasks.py:245/262/273`), not by `process_document_with_session` (the function this test calls directly, `tasks.py:83`) — so that assertion would never be reachable and would make the test always fail. Confirmed real coverage for the actual call already exists in `test_phase_e2_stability.py` (patches `_process_document_async` itself, asserts `mock_err.assert_called_once()`). Removed the not-actually-exercised `_mark_document_error` patch/capture from this test and added a comment pointing to where the behavior is genuinely verified, instead of fabricating an assertion outside this test's real scope.
- **Affected files**: `backend/tests/integration/test_phase7.py`, `backend/tests/integration/test_phase_d2.py`
- **Regression risk**: Low — test-only changes; full backend suite re-run after, 1763 passed/1 skipped/0 failed (unchanged), `ruff check` clean on both files.
- **Priority**: 49
- **Status**: **Closed — fixed** (2026-08-13)
- **Owner**: Engineering — closed
- **Verification method**: Source Verified (both test bodies + the real `_mark_document_error` call chain read in full before touching either test — the original recommendation for test 2 would have been incorrect) + Test Verified (both tests pass; the rate-limit test's new assertion exercises real behavior, not a tautology) + Regression Verified (full backend suite unchanged at 1763/1/0, ruff clean)
...
| ENG-049 | 2 tests assert less than their own docstring/comment promises | Low | 49 | **Closed — fixed (2026-08-13)** |
...
**2026-08-13 update**: ENG-049 closed (autonomous residual-issue pass) — see its entry above for why the fix differs from the originally-recommended assertion for the `test_phase_d2.py` case.
...
- **Closed: 35** — ENG-001,002,003,004,006,007,008,009,010,013,014,017,018,019,020,021,024,029,030,031,032,035,036,037,039,040,041,042,043,045,046,047,048,049,050
...
`35 + 7 + 3 + 1 + 4 = 50`. Zero unexplained entries: every item above is FIXED (closed), PROVEN FALSE (closed — no defect found / no longer reproducible), VERIFIED AND DEFERRED WITH A SPECIFIC REASON (deferred / reviewed-not-implemented / justified-not-changed), or CLASSIFIED AS A PRODUCT/INFRASTRUCTURE DECISION (open, with a decision record for ENG-033/034 in `docs/governance/`). No issue is untracked; no duplicate IDs exist; no closed issue appears open and no open issue appears closed anywhere else in this file as of this reconciliation. **All 4 remaining open items are genuinely blocked on external product/ops decisions or (ENG-038) an explicit "do not fix without reproduction" instruction from a prior sprint that still holds — re-verified this session, not re-litigated without new evidence.**
