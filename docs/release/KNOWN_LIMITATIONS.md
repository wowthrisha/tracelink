...
- **ENG-048 (new, V24.0, High severity)**: the Viewer's Reading Intelligence active-time counter resets to zero instead of pausing when the browser window loses focus, and restarts from zero instead of resuming on refocus. Confirmed real via 5+ independent Browser Verified reproductions; root cause not pinned despite a full source read of `useReadingAnalytics.js` — needs a live DevTools breakpoint session. **This is the top-priority open item in the entire backlog.** Full detail: `ENGINEERING_BACKLOG.md` ENG-048.
